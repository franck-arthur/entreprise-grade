...
# Ce script utilise les commandes CLI directement via docker exec
...
# Fonction pour exécuter des commandes Garage
garage_cmd() {
    docker exec enterprise-garage /garage -c /etc/garage.toml "$@"
}
...
NODE_ID=$(garage_cmd status 2>/dev/null | grep -E "^[a-f0-9]{16}" | head -1 | awk '{print $1}')

if [ -z "$NODE_ID" ]; then
    # Fallback: essayer de récupérer depuis les logs
    NODE_ID=$(garage_cmd status 2>/dev/null | awk '/HEALTHY NODES/,/^$/ {if(/^[a-f0-9]{16}/) print $1}' | head -1)
fi
...
if garage_cmd status | grep -q "NO ROLE ASSIGNED"; then
    echo "⚠️  Le nœud n'a pas de rôle assigné. Attribution en cours..."

    # Assigner un rôle au nœud
    garage_cmd layout assign "$NODE_ID" -z default -c 1G
...
if ! garage_cmd key list | grep -q "enterprise-app"; then
    echo "🔧 Création de la clé d'accès..."
    garage_cmd key create enterprise-app

    # Récupérer l'ID de la clé
    sleep 2
    KEY_ID=$(garage_cmd key list | grep enterprise-app | awk '{print $1}')

    if [ -n "$KEY_ID" ]; then
        echo "📋 ID de la clé: $KEY_ID"

        # Configurer les credentials
        garage_cmd key import "$KEY_ID" "$SECRET_KEY" -n enterprise-app --yes
...
if ! garage_cmd bucket list | grep -q "$BUCKET_NAME"; then
    echo "🔧 Création du bucket..."
    garage_cmd bucket create "$BUCKET_NAME"

    # Donner les permissions
    echo "🔐 Attribution des permissions..."
    garage_cmd bucket allow --read --write "$BUCKET_NAME" --key "$KEY_ID"
